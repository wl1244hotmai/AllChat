...
import sword.allchat.AllJoynService;
import sword.allchat.Observable;
import sword.allchat.Observer;
import sword.allchat.AllJoynService.UseChannelState;
...
public class ChatApplication extends Application implements Observable {
	...
	public synchronized void setUserName(String name){
		mUserName=name;
		notifyObservers(USER_CHANGED_EVENT);
	}
	...
	List<List<String>> mUserList = new ArrayList<List<String>>();
	List<String> mUsers = new ArrayList<String>();
	List<String> mUniqueName = new ArrayList<String>();

	public synchronized void clearFoundUser()
	{
		mUsers.clear();
	}
	
	public synchronized void addFoundUser(String user)
	{
		mUsers.add(user);
		Log.i(TAG, "addFoundUser(): add " + user);
	}
	
	public synchronized void removeLostUser(String user)
	{
        for (Iterator<String> i = mUsers.iterator(); i.hasNext();) {
...
//        	string = string.substring(index+1,string.length());   	
        	if (string.indexOf(user) != -1) {
            	JSONObject item = new JSONObject();
    				...
    				item.put("text", string + " has leaved this channel");
    			} catch (JSONException e) {
    				// TODO Auto-generated catch block
    				e.printStackTrace();
    			}
    			newRemoteUserMessage(item);
            	Log.i(TAG, "removeFoundUser(): removed " + user);
        		i.remove();
        	}
        }
	}
	
	public synchronized List<String> getFoundUsers()
	{
        List<String> clone = new ArrayList<String>(mUsers.size());
        for (String string : mUsers) {
        	Log.i(TAG, "getFoundUser(): get " + string);
	 ...
	 */
	public synchronized void newLocalUserMessage(JSONObject message) {
		if(message.optInt("type",-1)!=3)
		{
	 ...
	 */
	public synchronized void newRemoteUserMessage(JSONObject message) {
		addInboundItem(message);
	}
	 ...
	 */
	private List<JSONObject> mOutbound = new ArrayList<JSONObject>();
	 ...
	 */
	private void addOutboundItem(JSONObject message) {
	 ...
	 * sent.
	 */
	public synchronized JSONObject getOutboundItem() {
	 ...
	 */
	private void addInboundItem(JSONObject message) {
	 ...
	 */
	private List<JSONObject> mHistory = new ArrayList<JSONObject>();
	 ...
	 */
	private void addHistoryItem(JSONObject message) {
...
//      Date date = new Date();
		...
		Log.i(TAG, "addHistoryItem JSON message is:"+message);
	 ...
	 * deep copy.
	 */
    public synchronized JSONObject getNewItem() {
    	JSONObject clone = mHistory.get(0);
        Log.i(TAG,"getNewItem JSON is:"+mHistory.get(0));
        Log.i(TAG,"getNewItem JSON mHistory.size() is:"+mHistory.size());
        mHistory.remove(0);
        Log.i(TAG,"after getNewItem JSON mHistory.size() is:"+mHistory.size());
        return clone;
    }
