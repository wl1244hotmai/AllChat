...
import sword.allchat.TabWidget;
import sword.allchat.Observable;
import sword.allchat.Observer;
import sword.allchat.ChatInterface;
...
public class AllJoynService extends Service implements Observer {
	...
	List<String> joinerAdded = new ArrayList<String>();
	/**
    ...
    private void doBindSession() {
        ...
        Status status = mBus.bindSessionPort(contactPort, sessionOpts, new SessionPortListener() {
            ...
            public void sessionJoined(short sessionPort, int id, String joiner) {
                ...
                mHostSessionId = id;
                //主机的SessionListener
                if(isFirstJoin == 0 && mJoinedToSelf == true)
                {
                	Log.i(TAG,"BindSession() 绑定监听");
                	isFirstJoin = 1;
                	mBus.setSessionListener(mHostSessionId, new SessionListener() {
     					...
     					// Called by the bus for multipoint sessions when another
     					// node joins the session
     					public void sessionMemberAdded(int sessionId,
     							String uniqueName) {
     						
     						
     						Log.i(TAG, "sessionMemberAdded, sessionId is:"
     								+ sessionId + "  /  uniqueName is:"
     								+ uniqueName);
     						
     						//发现新加入用户后，尝试通知他自己的nickname
     						JSONObject requestItem =new JSONObject();
     						try {
								requestItem.put("type", 3);
								requestItem.put("username", uniqueName);
								requestItem.put("nickname", mChatApplication.getNickName());
							} catch (JSONException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
							}
     						mChatApplication.newLocalUserMessage(requestItem);
     						
//     						mChatApplication.addFoundUser(uniqueName);
//     						
//     						JSONObject item = new JSONObject();
//     						try {
//     							item.put("type", 2);
//     							item.put("nickname", "用户信息");
//								item.put("text", "User " + uniqueName + " has joined this channel");
//							} catch (JSONException e) {
//								// TODO Auto-generated catch block
//								e.printStackTrace();
//							}
//     						mChatApplication.newRemoteUserMessage(item);
     						
     					}

     					// Called by the bus for multipoint sessions when another
     					// node leaves the session.
     					public void sessionMemberRemoved(int sessionId,
     							String uniqueName) {
     						Log.i(TAG, "sessionMemberRemoved, sessionId is:"
     								+ sessionId + "  /  uniqueName is:"
     								+ uniqueName);
     						mChatApplication.removeLostUser(uniqueName);
                ...
                int flag=0;
				for (Iterator<String> i = joinerAdded.iterator(); i.hasNext();) 
				{
					String string = i.next();
					if (string.equals(joiner)) 
					{
						flag=1;
					}
				}
				if(flag==0){
	                joinerAdded.add(joiner);
				}
    ...
    private void doJoinSession() {
		...
		if (mHostChannelState != HostChannelState.IDLE) {
			if (mChatApplication.useGetChannelName().equals(
				...
				if(mHostSessionId != -1)
				{
					for (String uniqueName : joinerAdded) {
...
//					joinerAdded.clear();
        ...
        Status status = mBus.joinSession(wellKnownName, contactPort, sessionId, sessionOpts, new SessionListener() {
            ...
            public void sessionMemberAdded(int sessionId , String uniqueName)
            {
            	...
            	Log.i(TAG,"sessionMemberAdded, sessionId is:"+sessionId+"  /  uniqueName is:"+uniqueName);
            	//发现新加入用户后，尝试通知他自己的nickname
					JSONObject requestItem =new JSONObject();
					try {
					requestItem.put("type", 3);
            ...
            {
            	Log.i(TAG,"sessionMemberRemoved, sessionId is:"+sessionId+"  /  uniqueName is:"+uniqueName);
            	mChatApplication.removeLostUser(uniqueName);
    ...
    private void doSendMessages() {
        ...
        while ((jsonobject = mChatApplication.getOutboundItem()) != null) {
    ...
    public void Chat(String string) {
		...
		// String text="";
		try{
			item = new JSONObject(string);
			UserName = item.getString("username");
			nickname = item.getString("nickname");
			nickname = nickname + " " + ctx.sender;
        ...
        if(UserName.equals("Public") || UserName.equals(uniqueName) || UserName.equals(mChatApplication.getNickName()+" "+uniqueName))
        	;
        else
        {
        	Log.i(TAG, "Chat() dropped privacy siagal sent to "+UserName);
        	return;
        }
        
		if (item.optInt("type", -1) == 3) {
			mChatApplication.addFoundUser(nickname);

			JSONObject UserMessage = new JSONObject();
			try {
				UserMessage.put("type", 2);
				UserMessage.put("nickname", "用户信息");
				UserMessage.put("text", nickname + " has joined this channel");
			} catch (JSONException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			mChatApplication.newRemoteUserMessage(UserMessage);
         ...
         */
        //这里的
        String uniquename = ctx.sender;
        //uniquename = uniquename.substring(uniquename.length()-10, uniquename.length());
        
        //String nickname=mChatApplication.getNickName();
        Log.i(TAG, "Chat(): signal " + string + " received from nickname " + uniquename);
